...
optional<SubnetManager::SubnetInfo> SubnetManager::FindSubnet(vector<SubnetInfo>& group, const SockAddr_in info)
{
	const auto result = ranges::find_if(group
...
void SubnetManager::LoadSubnetGroup(String group, vector<SubnetInfo>& container)
{
	...
	for ( int i = 0; i < len; i++ )
	{
		...
		_subnetParser.GetValue(subnetString + to_wstring(i), subnet);
		_subnetParser.GetValue(ipString + to_wstring(i), ip);

		container.push_back({ subnet,Socket::Ip2Address(ip.c_str()),move(ip)});
	}

	ranges::sort(container, [](const SubnetInfo& d1, const SubnetInfo& d2)
	{
